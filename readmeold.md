...
|---|---|---|
| Virtual Speaker | **VS (speaker) by Xelph** | Playback endpoint — anything played to it is captured internally |
| Virtual Microphone | **VM (microphone) by Xelph** | Recording endpoint — plays back whatever the virtual speaker rendered |
...
- For checking Set ur default speaker to "Speaker (Virtual Audio Driver by Xelph)" and go to the sound control panel then recordings and you can see "Microphone (Virtual Audio Driver by Xelph) Vu Level and you can listen to it from the listen tab (make sure to select your real headphone from the playback device dropbox) 
...
- **Sound, video and game controllers** → "VS (speaker) by Xelph"
- **Audio inputs and outputs** → "VM (microphone) by Xelph"
...
Built on the Windows Driver Kit (WDK) and derived from Microsoft's Windows Driver Samples, specifically the Sysvad ("Simple Audio Sample") audio driver.
