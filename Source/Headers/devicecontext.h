...
Abstract:
    ...
    Declares IAdapterCommon (implemented by CAdapterCommon in
    Main/devicecontext.cpp) and the supporting types shared across the
    driver: control-flow helper macros (IF_FAILED_JUMP and friends), pool
    tags, the eDeviceType/PINTYPE enums, PIN_DEVICE_FORMATS_AND_MODES and
    ENDPOINT_MINIPAIR (which together describe an endpoint's pins, formats,
    and wave/topology miniport factories), and the loopback ring buffer
    accessors used to route speaker PCM into the microphone.
...
// If condition is TRUE, goto label.
...
// If condition is TRUE, run action then goto label.
...
// If ntStatus is a failure, run action then goto label.
...
// If ntStatus is a failure, goto label.
...
//
// Describes a pin's type (system, bridge, etc.), supported formats, and
// signal processing modes - conceptually similar to PCPIN_DESCRIPTOR but
// specific to this driver's implementation. Arrays of these follow the
// same order as the filter's pin descriptor array, so KS pin IDs double
// as the index.
//
...
// Both wave & topology miniport create functions have this signature.
...
typedef struct _AUDIO_DEVPROPERTY {
    const DEVPROPKEY   *PropertyKey;
    DEVPROPTYPE Type;
    ULONG BufferSize;
    __field_bcount_opt(BufferSize) PVOID Buffer;
} AUDIO_DEVPROPERTY, PAUDIO_DEVPROPERTY;
...
typedef struct _ENDPOINT_MINIPAIR
{
    ...
    // Topology miniport.
    PWSTR                           TopoName;               // make sure this or the template name matches with XELPH_VMS.<TopoName>.szPname in the inf's [Strings] section
    ...
    const AUDIO_DEVPROPERTY*       TopoInterfaceProperties;

    // Wave RT miniport.
    PWSTR                           WaveName;               // make sure this or the template name matches with XELPH_VMS.<WaveName>.szPname in the inf's [Strings] section
    ...
    const AUDIO_DEVPROPERTY*       WaveInterfaceProperties;
    ...
    // General endpoint flags (one or more ENDPOINT_<flag-type>, see above)
    ULONG                           DeviceFlags;
} ENDPOINT_MINIPAIR;
...
DECLARE_INTERFACE_(IAdapterCommon, IUnknown)
{
    ...
    //
    // Speaker -> microphone loopback ring buffer (see looprelay.h). Carries
    // PCM in the fixed hub format (16-bit stereo @ 48kHz). Safe to call at
    // up to DISPATCH_LEVEL.
    //
    STDMETHOD_(VOID,            LoopbackWrite)
    (
        THIS_
