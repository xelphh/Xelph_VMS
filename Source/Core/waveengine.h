...
Abstract:
    ...
    Declaration of CMiniportWaveRT, the WaveRT miniport class - the core
    streaming engine for each endpoint (speaker/microphone). Owns the
    endpoint's mix/device format, DRM rights, and the bookkeeping needed
    to create, validate and track individual CMiniportWaveRTStream
    instances (declared in streamworker.h) when the audio engine opens
    a stream on this filter's pins.
...
#ifndef _WAVEENGINE_H_
#define _WAVEENGINE_H_
