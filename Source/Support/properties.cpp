...
Abstract:
    ...
    Shared KS property handler implementations used by every filter: basic-
    support responses (KSPROPERTY_DESCRIPTION / access-flags-only), the
    volume/mute/peak-meter get/set handlers backed by CAdapterCommon's
    mixer state, CPU-resources reporting, waveformat extraction from a
    KSDATAFORMAT, and the PropertyDispatch/AUDIO_PROPERTY_ITEM table-driven
    dispatcher declared in properties.h.
...
// Extracts the WAVEFORMATEX from a WAVEFORMATEX/DSound-specifier KSDATAFORMAT;
...
)
{
    ...
    if (PropertyRequest && cbValueSize)
    {
        ...
        {
            // Caller is asking for the required ValueSize.
        ...
        else if (PropertyRequest->ValueSize >= cbValueSize)
        {
            ...
            {
                // Caller is responsible for setting ValueSize on success.
...
// Dispatches a request whose PropertyItem is an AUDIO_PROPERTY_ITEM to its
// Support/Get/Set handler (falling back to the generic basic-support handler
// if no SupportHandler is set).
#pragma code_seg("PAGE")
NTSTATUS
PropertyDispatch
(
...
)
{
    ...
    AUDIO_PROPERTY_ITEM* item = (AUDIO_PROPERTY_ITEM*)PropertyRequest->PropertyItem;
...
// Default basic-support handler: returns a full KSPROPERTY_DESCRIPTION if the
// buffer is large enough, otherwise just the access Flags as a ULONG.
#pragma code_seg("PAGE")
NTSTATUS
...
// Basic-support response for a Volume node: KSPROPERTY_DESCRIPTION plus a
// per-channel stepped range (0 dB to -96 dB in 0.5 dB steps).
#pragma code_seg("PAGE")
NTSTATUS
...
// Basic-support response for a Mute node: KSPROPERTY_DESCRIPTION plus a
// per-channel boolean stepped range.
#pragma code_seg("PAGE")
NTSTATUS
...
// Basic-support response for a peak-meter node: KSPROPERTY_DESCRIPTION plus a
// per-channel stepped range (GET/BASICSUPPORT only - peak meters are read-only).
#pragma code_seg("PAGE")
NTSTATUS
...
// Handles KSPROPERTY_AUDIO_CPURESOURCES - this driver's mixing/processing
// never runs on the host CPU (it's all in the "virtual hardware").
#pragma code_seg("PAGE")
NTSTATUS
...
// Handles KSPROPERTY_AUDIO_VOLUMELEVEL get/set, including the ALL_CHANNELS_ID
// broadcast case, backed by CAdapterCommon's mixer state.
#pragma code_seg("PAGE")
NTSTATUS
...
// Handles KSPROPERTY_AUDIO_MUTE get/set, including the ALL_CHANNELS_ID
// broadcast case, backed by CAdapterCommon's mixer state.
#pragma code_seg("PAGE")
NTSTATUS
...
// Handles KSPROPERTY_AUDIO_PEAKMETER2 get (read-only), backed by
// CAdapterCommon's mixer state.
#pragma code_seg("PAGE")
NTSTATUS
