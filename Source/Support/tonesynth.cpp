...
Abstract:
    ...
    Implementation of ToneGenerator: synthesizes a sine wave sample-by-sample
    into 8/16/24/32-bit PCM frames, carrying any partial frame across calls
    so capture buffers of arbitrary size still fill with continuous audio.
...
ToneGenerator::~ToneGenerator()
{
    if (m_PartialFrame)
    {
        ExFreePoolWithTag(m_PartialFrame, DRIVER_POOLTAG);
        m_PartialFrame = NULL;
        m_PartialFrameBytes = 0;
    }
}
