...
if exist "%VSWHERE%" (
    ...
    if defined MSBUILD_EXE (
        echo Found MSBuild via vswhere: !MSBUILD_EXE!
        goto msbuild_found
    )
    echo vswhere did not find an MSBuild install, searching common locations...
) else (
    echo vswhere.exe not found, searching common locations...
)
...
echo ERROR: MSBuild not found in PATH, via vswhere, or common installation locations
echo.
echo Searched locations:
echo   - System PATH
echo   - Visual Studio Installer (vswhere.exe)
echo   - Visual Studio 2022 Community/Professional/Enterprise/BuildTools
...
:: ApiValidator/package-verification are WHQL-style compliance checks, not
:: required to produce a working, loadable driver. They are disabled below
:: for x64 because they were failing on a missing WDK tool DLL (InfVerif.dll)
:: - this machine's WDK install ended up split across two mismatched SDK
:: versions, and the km/KMDF/bin content was manually merged from
:: 10.0.28000.0 into 10.0.26100.0 to get everything else working.
:: SkipPackageVerification is the property that actually gates the
:: DPVerifierTask call that loads InfVerif.dll (InfVerif_Enable is not a
:: real property in this WDK version's targets - it was a no-op). If you
:: later get a clean, matched WDK+SDK install, drop the two overrides below
:: to get full validation back.
if /i "%PLATFORM%"=="ARM64" (
    echo Building ARM64 with validation disabled...
    call "!MSBUILD_CMD!" "Xelph_VMS.sln" ^
    ...
    echo Building x64...
    call "!MSBUILD_CMD!" "Xelph_VMS.sln" /p:Configuration=%CONFIG% /p:Platform=%PLATFORM% /p:ApiValidator_Enable=false /p:SkipPackageVerification=true
)
...
echo Output directory: %PLATFORM%\%CONFIG%\Setup
goto show_output
...
for %%c in (%CONFIGS%) do (
    for %%p in (%PLATFORMS%) do (
        ...
        if /i "%%p"=="ARM64" (
            ...
            echo Building x64...
            call "!MSBUILD_CMD!" "Xelph_VMS.sln" /p:Configuration=%%c /p:Platform=%%p /p:ApiValidator_Enable=false /p:SkipPackageVerification=true
        )
...
if %BUILD_ALL%==1 (
    echo   x64\Debug\Setup\
    echo   x64\Release\Setup\
    echo   ARM64\Debug\Setup\
    echo   ARM64\Release\Setup\
) else (
    echo   %PLATFORM%\%CONFIG%\Setup\
)
echo.
echo Key files:
echo   - Xelph_VMS.sys  (driver binary)
echo   - Xelph_VMS.inf  (installation file)
echo   - xelph_vms.cat  (catalog file)
